...
def detect_and_draw_shape(points, canvas, color, thickness):
    ...
    if circle_ratio > 0.70:
        ...
        if 0.90 <= aspectRatio <= 1.10: 
            ...
            cv2.rectangle(canvas, (center_x - side//2, center_y - side//2), (center_x + side//2, center_y + side//2), color, thickness)
        else: 
            cv2.rectangle(canvas, (x, y), (x + w, y + h), color, thickness)
    else:
        extent = area / float(w * h)
        if extent > 0.8: cv2.rectangle(canvas, (x, y), (x + w, y + h), color, thickness)
        else: cv2.polylines(canvas, [contour], False, color, thickness)
...
class AirCanvasProcessor(VideoProcessorBase):
    def __init__(self):
        ...
        try:
            ...
            self.hands = self.mpHands.Hands(min_detection_confidence=0.6, min_tracking_confidence=0.5, max_num_hands=1)
            self.mp_loaded = True
    ...
    def draw_ui(self, img):
        ...
        cv2.circle(img, (450, 50), 30, self.currColor, cv2.FILLED)
        ...
        val = int(np.interp(self.brushThickness, [1, 150], [SLIDER_X1, SLIDER_X2]))
        cv2.circle(img, (val, SLIDER_Y), 12, (255, 255, 255), cv2.FILLED)
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        if not self.mp_loaded:
            cv2.putText(img, "Error: LibGL missing. Check packages.txt", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        ...
        try:
            ...
            if results.multi_hand_landmarks:
                for handLms in results.multi_hand_landmarks:
                    ...
                    if len(lmList) != 0:
                        raw_x1, raw_y1 = lmList[8][1:]
                        x1 = int(self.prev_x + (raw_x1 - self.prev_x) * self.smoothing_factor)
                        y1 = int(self.prev_y + (raw_y1 - self.prev_y) * self.smoothing_factor)
                        ...
                        if fingers[1] and fingers[2]:
                            ...
                            if y1 < HEADER_HEIGHT:
                                if SPEC_X1 < x1 < SPEC_X2:
                                    ...
                                    self.shapePoints = []
                                elif SLIDER_X1 < x1 < SLIDER_X2:
                                    val = np.interp(x1, [SLIDER_X1, SLIDER_X2], [1, 150])
                                    if self.currColor == (0,0,0): self.eraserThickness = int(val)
                                    else: self.brushThickness = int(val)
...
st.set_page_config(page_title="Air Canvas Pro", layout="wide")
